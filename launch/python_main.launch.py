...
def generate_launch_description():
    ...
    # TextSubstitution(text="0.0") will only evaluate at execution time.

    turning_speed_arg = DeclareLaunchArgument(
        "turning_speed", default_value=TextSubstitution(text="0.2")  # TextSubstitutions evaluate at run time
    )
    forward_speed_arg = DeclareLaunchArgument( 
        "forward_speed", default_value="0.1"    # Note: More often, without TextSubstitution
    )
    ...
    turning_speed_f = LaunchConfiguration('turning_speed')  # future value of arg, evaluated at run time
    ...
    start_rviz_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            'rviz_config_file_name': rviz_config_file_name_f}.items()  # convert to list!
    )

    # include another launch file in the chatter_ns namespace ??? mazinger?
    ...
    return LaunchDescription([  # these are all launch.actions
        turning_speed_arg,
        forward_speed_arg,
